...
export default function HistoryScreen() {
  ...
  const { history, streak, longestStreak } = useFasting();

  const topPadding = Platform.OS === "web" ? 67 + 16 : insets.top + 16;
  const bottomPadding = Platform.OS === "web" ? 34 + 24 : 24;

  const completedFasts = history.filter(
    (d) => d.status === "completed" && d.type === "fast"
  ).length;
  const completedEats = history.filter(
    (d) => d.status === "completed" && d.type === "eat"
  ).length;
  ...
  return (
    ...
    >
      ...
      <View style={[styles.calendarCard, { backgroundColor: colors.card, shadowColor: "#000" }]}>
